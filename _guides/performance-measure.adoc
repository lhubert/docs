...
* which additional flags will Logisland apply to native-image by default
...
[source, shell]
--
$ date +"%T.%3N" &&  ./target/logisland-timing-runner
...
== Additional flags applied by Logisland

When Logisland invokes GraalVM `native-image` it will apply some additional flags by default.
...
Logisland disables this feature by setting `-H:FallbackThreshold=0`: this will ensure you get a compilation failure rather
...
Isolates are a neat feature of GraalVM, but Logisland isn't using them at this stage.

Disable via `-H:-SpawnIsolates`.

=== Disable auto-registration of all Service Loader implementations

Logisland extensions can automatically pick the right services they need, while GraalVM's native-image defaults to include
...
Logisland defaults to server applications, so we switch to a better default by setting
...
If you're to investigate some differences in detail make sure to check what Logisland is invoking exactly: when the build
