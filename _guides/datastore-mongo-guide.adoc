...
Learn how to send {project-name} `Records` into MongoDB through de the Datastore API
...
For this guide we need a {project-name} stack (Zookeeper, Kafka, Logisland) and an MongoDB as a complete Docker compose config.
...
You should notice that `MongoDB_service` is the unique name given to the ControllerService which can referenced by any Processor config further in the config file.
